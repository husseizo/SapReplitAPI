...
");
        ...
        IDictionary<int, InvoiceLifecycleEvidence> evidenceByDocEntry)
    {
        ...
        {
            rs = (SAPbobsCOM.Recordset)company.GetBusinessObject(BoObjectTypes.BoRecordset);
            rs.DoQuery(SapReplitAPI.Services.Returns.InvoiceReturnsSql.CreditMemoEvidence(string.Join(",", docEntries)));
            while (!rs.EoF)
            {
                if (evidenceByDocEntry.TryGetValue(GetInt(rs, "InvoiceDocEntry"), out var evidence))
                {
